/**
 * Command Center — analytics matrix (Agihan MN: PAS/PN 13 · UMNO/BN 23)
...
(function () {
  ...
  function buildSeat(code, name, war, cula, soc, targetMeta) {
    ...
    return {
      ...
      _raw: { bkc, delta24, culaPct, mentions24, negPct, soc },
      socmed: normalizeSocmed(soc),
    };
  }

function normalizeSocmed(soc) {
    if (!soc || !soc.mentions_total) return null;
    return {
      lean: soc.lean, leanLabel: soc.lean_label, emotionTop: soc.emotion_top,
      emotion: soc.emotion, issues: soc.top_issues, confidence: soc.confidence,
      mentions: soc.mentions_total, negPct: soc.neg_pct, posPct: soc.pos_pct,
      demoMix: soc.demo_mix || {}, demoTop: soc.demo_top, demoInsight: soc.demo_insight,
      narrativeSplitPct: soc.narrative_split_pct,
    };
  }
  ...
  const LEAN_TAG = {
    "Cenderung Sokong": { cls: "lean-pos", short: "Sokong" },
    "Majoriti Atas Pagar": { cls: "lean-mid", short: "Atas pagar" },
    "Cenderung Menentang": { cls: "lean-neg", short: "Menentang" },
    "Tiada data": { cls: "lean-muted", short: "—" },
  };
  const EMO_LABEL = {
    anger: "marah", fear: "takut", sadness: "sedih",
    happy: "gembira", love: "kasih/sokong", surprise: "terkejut", neutral: "neutral",
  };

  function leanCell(s) {
    const soc = s._raw.soc;
    if (!soc || !soc.mentions_total) return `<span class="cell-muted">—</span>`;
    const tag = LEAN_TAG[soc.lean_label] || LEAN_TAG["Tiada data"];
    const conf = soc.confidence && soc.confidence !== "Tinggi" ? ` <span class="lean-conf">(${soc.confidence})</span>` : "";
    return `<span class="lean-tag ${tag.cls}">${tag.short}</span>${conf}`;
  }

  function buildStateSocmedBullets() {
    const r = socmedCache?.rollup;
    if (!r || !r.state_lean) return [];
    const L = r.state_lean;
    const issues = (r.top_issues_state || []).slice(0, 3).map((i) => i.issue).join(", ");
    const emo = Object.entries(r.state_emotion || {}).sort((a, b) => b[1] - a[1])[0];
    const emoTxt = emo ? `${EMO_LABEL[emo[0]] || emo[0]} ${emo[1]}%` : "—";
    const scanned = socmedCache?.meta?.state_rows_scanned || 0;
    const bullets = [{
      level: L.menentang >= 20 ? "warn" : "ok",
      text: `<strong>Nadi socmed negeri</strong> (${fmt(scanned)} mesej NS): Sokong ${L.sokong}% · Atas pagar <strong>${L.atas_pagar}%</strong> · Menentang ${L.menentang}%.`,
    }];
    if (issues) {
      bullets.push({ level: "ok", text: `<strong>Isu panas socmed:</strong> ${issues} · emosi dominan: ${emoTxt}.` });
    }
    return bullets;
  }
  ...
  function renderMatrix(seats) {
    return `
      ...
      <div class="pas-matrix-foot">
        <button type="button" class="btn-sm btn-primary" id="cmdOpenDunMap">Buka peta kerusi →</button>
  ...
  function renderIntelFeed(culaHub) {
    ...
    const brief = buildExecutiveBrief(seats, culaHub).concat(buildStateSocmedBullets());
    ...
    if (metaEl) {
      ...
      const tm = pasTargetCache?.meta;
      const parts = [];
      parts.push("Agihan MN: PAS/PN 13 · UMNO/BN 23");
  ...
  window.CommandIntel = {
    getSeatPlaybook,
    getSocmedForDun(codeOrId) {
      const byDun = socmedCache?.byDun || {};
      const raw = normCode(codeOrId);
      const m = raw.match(/^N(\d+)$/);
      const cands = [raw];
      if (m) {
        cands.push("N" + m[1].padStart(2, "0"));
        cands.push("N" + String(parseInt(m[1], 10)));
      }
      for (const k of cands) {
        if (byDun[k]) return normalizeSocmed(byDun[k]);
      }
      return null;
    },
    ...
    exportWhatsAppText(seat, playbook) {
      if (!playbook) return "";
      const sm = playbook.socmed;
      const smLine = sm && sm.mentions
        ? `Socmed: ${sm.leanLabel} (${sm.mentions} mention, keyakinan ${sm.confidence})`
            + (sm.issues && sm.issues.length ? ` · isu: ${sm.issues.map((i) => i.issue).join(", ")}` : "")
        : "";
      const lines = [
