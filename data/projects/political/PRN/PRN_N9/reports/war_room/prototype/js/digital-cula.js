/**
 * InsightPulse — Laporan Cula Harian (N9)
...
(function () {
  ...
  async function refreshActionCenterAfterCula() {
    if (typeof window.refreshActionCenterFromCula === "function") {
      await window.refreshActionCenterFromCula();
    }
  }
  ...
  async function loadCulaHub() {
    try {
      ...
      culaHub = data.hub;
      if (data.action_center) await refreshActionCenterAfterCula();
    } catch {
  ...
  async function submitCulaForm(ev) {
    ...
    try {
      ...
      msg.textContent = data.message || "✓ Data diterima — Command Center & Action Center dikemaskini.";
      msg.classList.add("ok");
      if (typeof showToast === "function") showToast("✓ Cula dikemaskini — Action Center");
      $("culaForm").reset();
  ...
  async function uploadBatch() {
    ...
    let body;
    const defaultTarikh = $("culaBatchTarikh")?.value || todayISO();
    if (file) {
      const csv = await file.text();
      body = { state, uploader, mode: "csv", csv, tarikh_lawatan: defaultTarikh };
    } else if (paste) {
      body = {
        state,
        uploader,
    ...
    try {
      ...
      msg.textContent = `✓ ${data.imported} rekod diproses — Command Center & Action Center dikemaskini.`;
      msg.classList.add("ok");
      if (data.errors?.length) {
        log.textContent = data.errors.join("\n");
        log.classList.remove("hidden");
      }
      if (typeof showToast === "function") showToast(`✓ ${data.imported} rekod — Action Center dikemaskini`);
      $("culaCsvFile").value = "";
      $("culaWhatsappPaste").value = "";
      await loadCulaHub();
      if (typeof refreshProductionData === "function") await refreshProductionData();
      if (typeof window.refreshDunDrawerAfterCula === "function") await window.refreshDunDrawerAfterCula();
    } catch (e) {
      msg.textContent = e.message || "Upload gagal.";
      msg.classList.add("err");
    } finally {
      if (btn) { btn.disabled = false; btn.textContent = "Proses & Kemaskini Dashboard"; }
    }
  }
  ...
  window.initDigitalCula = async function initDigitalCula() {
    ...
    const culaStartTab = culaParams.get("tab") || "upload";
