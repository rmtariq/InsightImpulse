...
# Get filename from command line or use latest
csv_file = sys.argv[1] if len(sys.argv) > 1 else 'data/combined/Combined_linkedin_20260529_115332.csv'
...
if len(comments_without_parent) > 0:
    ...
    print(comments_without_parent[['ID', 'Text', 'Parent_Post_ID']].head(3))
    print()

print("=" * 80)

# Find posts that have comments
posts_with_comments = comments_with_parent['Parent_Post_ID'].unique()
...
for parent_id in posts_with_comments:
    ...
    parent_post = posts[posts['ID'] == str(parent_id)]
    
    if len(parent_post) == 0:
        # Post might not be in our dataset (comment orphaned)
        print(f"\n🔗 Parent Post ID: {str(parent_id)[:30]}... (NOT in dataset - orphaned)")
    else:
    ...
    post_comments = comments_with_parent[comments_with_parent['Parent_Post_ID'] == parent_id]
...
print(f"\n✅ Parent-child linking is working for LinkedIn!")
