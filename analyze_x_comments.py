...
# Load the CSV
df = pd.read_csv('data/combined/Combined_x_20260529_104313.csv')

# Separate posts and comments
posts = df[df['Type'] == 'post'].copy()
comments = df[df['Type'] == 'comment'].copy()

print(f"📊 X/TWITTER TEST RESULTS")
print(f"=" * 80)
...
for parent_id in posts_with_comments:
    ...
    parent_post = posts[posts['ID'].astype(str) == parent_id_str]
    
    if len(parent_post) == 0:
        # Post might not be in our dataset (comment orphaned)
        print(f"\n🔗 Parent Post ID: {parent_id_str[:20]}... (NOT in dataset - orphaned)")
    else:
...
print(f"\n✅ Parent-child linking is working for X/Twitter!")
